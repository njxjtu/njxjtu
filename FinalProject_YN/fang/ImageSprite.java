...
{        
     ...
     */
    public ImageSprite(URL url, boolean keepAspect)
    {
    	this.keepAspect=keepAspect;
    	initialize(url);
    }
    ...
    private boolean initializeImage(URL filename)
    {
        ...
        } else
        {
            try
            {
            	ImageInputStream x=ImageIO.createImageInputStream(filename.openStream());
            	Iterator<ImageReader> reader=ImageIO.getImageReaders(x);
            	ImageReader one=reader.next();
            	one.setInput(x);
            	image=one.read(0);
/*            	System.err.println("filename is "+filename.getFile());
            	System.err.println("making icon");
                ImageIcon icon = new ImageIcon(filename);
                System.err.println("made icon");
                if(icon.getImageLoadStatus()==MediaTracker.ABORTED || 
                		icon.getImageLoadStatus()==MediaTracker.ERRORED)
                {
                	ErrorConsole.addError(
                			"The image "+filename.getFile()+
                			"you are trying to load is not in " +
                			"a format that Java can load.", 
                			"Try opening the image in GIMP or other image " +
                			"editing software and saving the image as a gif or jpg.", 
                			new Exception("Bad Image Format"));
                	return false;
                }
                else
                {
...
//                System.err.println("image load status: "+icon.getImageLoadStatus());
//                System.err.println("aborted: "+MediaTracker.ABORTED);
//                System.err.println("errored: "+MediaTracker.ERRORED);
//                System.err.println("complete: "+MediaTracker.COMPLETE);
                image = icon.getImage();*/
            }
            catch(Exception e)
            {
    ...
    private void updateBufferedImage()
    {
                ...
                flipchange)
        {
            ...
            } else
            {
                ...
                buffered = new BufferedImage(size.x, size.y, BufferedImage.TYPE_INT_ARGB);
                double scale=((double)buffered.getHeight(null))/image.getHeight(null);
                ((Graphics2D)buffered.getGraphics()).drawImage(image, AffineTransform.getScaleInstance(scale, scale), null);
                if(flip.x<0 || flip.y<0)
                {
                    BufferedImage buff=new BufferedImage(
                            size.x, size.y,
